...
var require_app = __commonJS({
  "commands/app.js"(exports2, module2) {
    function run() {
      const { log } = require_index();
...
var require_init = __commonJS({
  "commands/init.js"(exports2, module2) {
    var readline = require("readline");
    var interface2 = readline.createInterface({
      input: process.stdin,
    ...
    function ask(question, regex = null) {
      return new Promise(async (resolve) => {
        interface2.question(question, async (answer) => {
          if (!answer.trim()) {
    ...
    async function run() {
      ...
      const authorsInput = (await ask(style("Authors (comma-separated): ", "#468dff"))).trim();
      const author = authorsInput.split(",").map((s) => s.trim()).filter(Boolean);
      const packageID = (await ask(style("Package ID (e.g. website.mod.author): ", "#468dff"), /^[a-zA-Z0-9]+(\.[a-zA-Z0-9]+){2}$/)).trim();
      const game = (await ask(style("Game code: ", "#468dff"), /^[a-zA-Z0-9]+$/)).trim();
      const url = (await ask(style("URL: ", "#468dff"), /^https?:\/\/.+/)).trim();
      ...
      const tagsInput = (await ask(style("Tags (comma-separated): ", "#468dff"), /^[a-zA-Z0-9\s\-_]+$/)).trim();
      const tags = tagsInput.split(",").map((s) => s.trim()).filter(Boolean);
      const deltaruneTargetVersion = packageID.startsWith("toby.deltarune") ? (await ask(style("Deltarune target version: ", "#468dff"), /^\d+\.\d+\.\d+$/)).trim() : "";
      const data = {
        metadata: {
          name: name || "example",
        ...
        deltaruneTargetVersion: deltaruneTargetVersion || "1.04",
        exporter: {
          tool: "deltamodCLI"
        }
      };
...
Wrote meta.json to ${outPath}`, "#84ff9f"));
      interface2.close();
    }
    module2.exports = run;
  }
...
// node_modules/app-data-folder/index.js
var require_app_data_folder = __commonJS({
  "node_modules/app-data-folder/index.js"(exports2, module2) {
    var path = require("path");
    var fs = require("fs");
    function appData(...app) {
      let appData2;
      if (process.platform === "win32") {
        appData2 = path.join(process.env.APPDATA, ...app);
      } else if (process.platform === "darwin") {
        appData2 = path.join(process.env.HOME, "Library", "Application Support", ...app);
      } else {
        appData2 = path.join(process.env.HOME, ...prependDot(...app));
      }
      return appData2;
    }
    ...
    module2.exports = appData;
  }
});

// commands/import.js
var require_import = __commonJS({
  "commands/import.js"(exports2, module2) {
    function run() {
      const appdata = require_app_data_folder();
      let appDataPath = appdata("Deltamod");
      const { log, style } = require_index();
      const fs = require("fs");
      ...
      fs.mkdirSync(path.join(appDataPath, "pkg.db", "Mod-" + t), { recursive: true });
      fs.copyFileSync(process.cwd(), path.join(appDataPath, "pkg.db", "Mod-" + t));
      log("import complete");
    }
    module2.exports = run;
  }
